...
- Click card → onSelect (activates content piece)
...
## Search (merged from Story 6.1)

The pipeline sidebar also serves as the content library (Library tab removed).
Add a search input at the top of the sidebar that filters content across all sections by title and path.
...
- [ ] Search input filters content across all sections (merged from 6.1)
