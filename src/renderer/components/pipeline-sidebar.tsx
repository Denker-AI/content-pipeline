...
export function PipelineSidebar({
  ...
  const toggleBranches = (repoPath: string) => {
          ...
          <div className="min-h-0 flex-1 overflow-y-auto thin-scrollbar">
            {repos.map((repo) => (
              <RepoSection
                ...
                branchesExpanded={expandedBranches.has(repo.path)}
                onToggle={() => toggleRepo(repo.path)}
                onToggleBranches={() => toggleBranches(repo.path)}
                onRemove={() => handleRemoveRepo(repo.path)}
                confirmRemove={confirmRemoveRepo === repo.path}
                filteredItems={filterItems(repo.items)}
                groupedItems={repos.length <= 1 ? groupedItems : groupItems(filterItems(repo.items))}
...
function RepoSection({
  ...
  const contentWorktrees = repo.worktrees.filter((wt) => wt.branch.startsWith('content/'))
    ...
    <div className="border-b border-zinc-200 dark:border-zinc-700">
      ...
      {isExpanded && (
        <div className="pl-2">
          ...
          {contentWorktrees.length > 0 && (
            <div>
              <button
                onClick={onToggleBranches}
                className="flex w-full items-center gap-1.5 px-2 py-1.5 text-left hover:bg-zinc-50 dark:hover:bg-zinc-800/50"
              >
                {branchesExpanded ? (
                  <ChevronDownIcon className="h-3 w-3 shrink-0 text-zinc-400" />
                ) : (
                ...
                <GitBranchIcon className="h-3 w-3 shrink-0 text-zinc-400" />
                <span className="text-[11px] font-medium text-zinc-500 dark:text-zinc-400">
                  Branches
                </span>
                <span className="text-[10px] text-zinc-400 dark:text-zinc-500">
                  {contentWorktrees.length}
                </span>
              </button>
              {branchesExpanded && (
                <div className="pl-4">
                  {contentWorktrees.map((wt) => (
                    <div
                      key={wt.path}
                      className="flex cursor-pointer items-center gap-2 px-2 py-1 hover:bg-zinc-50 dark:hover:bg-zinc-800/50"
